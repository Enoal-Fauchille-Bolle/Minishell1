...
static void parse_format(va_list list, char *str, int *i, int *counter)
{
    ...
    if (end != -1) {
        *i += end - start;
        SPECIFIER_FUNCTIONS[my_str_get_index(SPECIFIER_CALLS, str[end])](list,
            counter,
            separate_modifiers_from_str(my_slice(str, start + 1, end - 1)));
    }
}

static void parse_char(va_list list, char *str, int *i, int *counter)
{
    if (str[*i] == '%') {
